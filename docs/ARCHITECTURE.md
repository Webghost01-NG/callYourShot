...
Issue #78 makes bounded leaderboard membership deterministic. Supabase may
store owner-published score snapshots with a formula version, exact rational,
sample count, evidence block, and server timestamp, but these claims never
choose which wallets are checked. The browser walks enrollment-time and wallet-
address cohorts of at most 24, accumulates the active coverage cycle, and ranks
only fresh DreamDEX rebuilds. Stored scores never enter the ranking reducer. See
